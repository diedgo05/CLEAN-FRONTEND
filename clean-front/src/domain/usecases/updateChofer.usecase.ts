...
export class updateChoferUseCase implements UseCase <{id: number, nombre: string, apellido_p: string, apellido_m: string, edad: number}, ChoferModel> {
    ...
    execute(params: { id: number; nombre: string; apellido_p: string; apellido_m: string; edad: number; }): Observable<ChoferModel> {
        return this.cRepo.update(params);
    }
}